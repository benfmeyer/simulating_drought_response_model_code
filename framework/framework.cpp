...
int framework(const CommandLineArguments& args) {
	...
	const InputModuleType input_module_type =  args.get_input_module_type();
	
	std::unique_ptr<InputModule> input_module(InputModuleRegistry2::CreateInstance(input_module_type));
	...
	input_module->init(args);

	output_modules.init(input_module.get(), args.get_verbose());
	...
	unique_ptr<GuessSerializer> serializer;
    unique_ptr<GuessDeserializer> deserializer;

	if (save_state) {
		serializer = unique_ptr<GuessSerializer>(new GuessSerializer(state_path, Parallel::get().get_rank(), Parallel::get().get_num_processes()));
	}

	if (restart) {
		deserializer = unique_ptr<GuessDeserializer>(new GuessDeserializer(state_path));
	}
...
#ifdef HAVE_SMARTIO
void control_processes_auto(const CommandLineArguments& args)
{
	#ifdef HAVE_MPI
	// The 'mission control' of the model, responsible for maintaining the
	// primary model data structures and containing all explicit loops through
	...
	InputModuleType input_module_type = args.get_input_module_type();

	unique_ptr<InputModule> input_module(InputModuleRegistry2::CreateInstance(input_module_type));

	// Initializer controler to account for output parameters
	Controler controler;

	// Read intruction file to get the list of coordinates
	read_instruction_file(args.get_instruction_file());

	input_module->init(args);

	NcInputParallelAutoGuess* par_ptr = dynamic_cast<NcInputParallelAutoGuess*>(input_module.get());

	par_ptr->SendReceiveHandling();
#endif // HAVE_MPI
}

void control_processes_auto_single(const CommandLineArguments& args)
{
#ifdef HAVE_MPI
	// The 'mission control' of the model, responsible for maintaining the
	// primary model data structures and containing all explicit loops through
	...
	NcInputParallelAutoSingleGuess* par_ptr = dynamic_cast<NcInputParallelAutoSingleGuess*>(input_module.get());

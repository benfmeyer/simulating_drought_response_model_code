...
/// Implementation of Hill equation for loss of conductivity calculation
/// The hill equation is also known as polynomial/logisitic response curve

class HillEquation {
public:
	HillEquation(const double slope, const double p50, bool use_powQ = false);
	~HillEquation();

	double Get(const double px);
	double GetD(const double px);
private:

	double GetStd(const double px);
	double slope;
	double p50;
...
// Implementation of the weibull culumative distribution function
class WeibullCDF
{
	double shape;
	double scale;
...
public:
	...
	WeibullCDF(double shape, double scale) :shape(shape), scale(scale)
	{
...
class MCWD_Calculator
{
public:
	MCWD_Calculator(Climate& climate);
	~MCWD_Calculator();
...
private:
	...
	/// Climate reference, here read and write, because we need to adjust values of the climate
	Climate& climate_;

...
Patch::Patch(int i,Stand& s,Soiltype& st):
	...
	n_indivs_alive_pft.resize(pftlist.nobj);
...
void PlantHydraulics::serialize(ArchiveStream &arch) {
    arch & psi_sw
         & psi_sw_prev
         & psi_sw_min
         & kappa_s_min;
...
PlantHydraulics::PlantHydraulics() {

	// On the first day of existence assume no cavitation
	kappa_L = 0.0;
	kappa_r = 0.0;
	kappa_s_today = 0.0;
    ...
    gC_water = 0.0;
    gC = 0.0;
    waterStressedGC = false;
    et_plant_total = 0.0;
    low_diffusion_rate_w = false;
...
Individual::Individual(int i,Pft& p,Vegetation& v):pft(p),vegetation(v),id(i) {
	...
	// --> PPX-Hydraulics
	// Initialise lambda with maximum stomatal opening
	lambda = pft.lambda_max;

	switch (hydraulic_system)
	{
		case STANDARD:
			hydraulic_module = std::make_unique<HydroDynamicsEmpty>(*this);
			break;

		case VPD_BASED_GC:
			hydraulic_module = std::make_unique<HydroDynamicsVPD>(*this);
			break;

		default:
			fail("Invalid hydraulic system!");
	}
	// <-- PPX-Hydraulics
...
void Individual::serialize(ArchiveStream& arch) {
	...
	// --> PPX-Hydraulics
	//hydro_vpd_ptr = nullptr;
	//hydro_month_ptr = nullptr;
...
void Individual::reduce_biomass(double mortality, double mortality_fire, bool bfire, bool bgreff, double mort_ratio, double mort_bg, double mort_greff, double mort_cav) {
	...
	if (!negligible(mortality)) {
		...
		report_flux(Fluxes::FIREC_NO_LITTER,    cflux_fire);
		report_flux(Fluxes::INDIV_FIREC,    cflux_fire);
		...
		const double total_carbon_mort_loss = cmass_leaf_litter + cmass_root_litter + mortality * cmass_wood();
		const double fraction_mort_sum = mort_bg + mort_greff + mort_cav;

		if (!negligible(fraction_mort_sum))
		{
			//Note: This is different to the actual mortality value!
			ppft.cmass_lost_bg += total_carbon_mort_loss * mort_bg / fraction_mort_sum;
			ppft.cmass_lost_greff += total_carbon_mort_loss * mort_greff / fraction_mort_sum;
			ppft.cmass_lost_cav += total_carbon_mort_loss * mort_cav / fraction_mort_sum;

			ppft.cmass_lost_heart_bg += cmass_heart * mort_bg / fraction_mort_sum * mortality;
			ppft.cmass_lost_heart_greff += cmass_heart * mort_greff / fraction_mort_sum * mortality;
			ppft.cmass_lost_heart_cav += cmass_heart * mort_cav / fraction_mort_sum * mortality;

			ppft.cmass_lost_sap_bg += cmass_sap * mort_bg / fraction_mort_sum * mortality;
			ppft.cmass_lost_sap_greff += cmass_sap * mort_greff / fraction_mort_sum * mortality;
			ppft.cmass_lost_sap_cav += cmass_sap * mort_cav / fraction_mort_sum * mortality;

			ppft.cmass_lost_leaf_bg += cmass_leaf * mort_bg / fraction_mort_sum * mortality;
			ppft.cmass_lost_leaf_greff += cmass_leaf * mort_greff / fraction_mort_sum * mortality;
			ppft.cmass_lost_leaf_cav += cmass_leaf * mort_cav / fraction_mort_sum * mortality;

			ppft.cmass_lost_root_bg += cmass_root * mort_bg / fraction_mort_sum * mortality;
			ppft.cmass_lost_root_greff += cmass_root * mort_greff / fraction_mort_sum * mortality;
			ppft.cmass_lost_root_cav += cmass_root * mort_cav / fraction_mort_sum * mortality;
...
size_t Gridcell::get_id() const
{
	return id;
}

std::string Gridcell::get_description() const
{
    return description;
}

size_t Gridcell::get_index() const
{
	return index;
}

size_t Gridcell::get_n_proc_gridcells() const
{
	return n_proc_gridcells;
}

size_t Gridcell::get_n_total_gridcells() const
{
	return n_total_gridcells;
}

size_t Gridcell::get_first_year() const
{
	return first_simulation_year;
}

size_t Gridcell::get_last_year() const
{
...
void Gridcell::set_index(size_t index)
{
	this->index = index;
}

void Gridcell::set_id(size_t id)
{
	this->id = id;
}

void Gridcell::set_total_counts(size_t n_proc_cells, size_t n_total_cells)
{
	this->n_proc_gridcells = n_proc_cells;
	this->n_total_gridcells = n_total_cells;
}

void Gridcell::set_time_begin_end(int first_year, int last_year)
{
	first_simulation_year = first_year;
	last_simulation_year = last_year;
}
...
void Gridcell::set_description(std::string basicString) {

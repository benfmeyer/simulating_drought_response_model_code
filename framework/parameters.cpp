...
// --> PPX-Hydraulics
hydraulicsystemtype hydraulic_system;
bool disable_mort_greff = false;
double alphaa_nlim;
// <-- PPX-Hydraulics
...
#ifdef HAVE_SMARTIO
SmartOutputBuilderOptions so_options;
#endif
...
enum {CB_NONE,CB_VEGMODE,CB_CHECKGLOBAL,CB_LIFEFORM,CB_LANDCOVER,CB_PHENOLOGY,CB_LEAFPHYSIOGNOMY,CB_SELECTION,	
	...
	CB_PATHWAY, CB_ROOTDISTRIBUTION, CB_ROOTFRAC, CB_EST, CB_CHECKPFT, CB_STRPARAM, CB_NUMPARAM, CB_WATERUPTAKE, CB_HYDRAULICSYSTEM, CB_SMARTOUT_TIMERANGE, CB_MTCOMPOUND,
	CB_FIREMODEL,CB_WEATHERGENERATOR, CB_OUT_DATA_FORMAT};
...
void plib_declarations(int id,xtring setname) {
	...
	case BLOCK_GLOBAL:
        ...
        // --> PPX-Hydraulics
        declareitem("hydraulic_system", &strparam, 20, CB_HYDRAULICSYSTEM,
                        "Hydraulic architecture system (\"STANDARD\", \"MONTEITH_SUP_DEM\", \"VPD_BASED_GC\")");

        declareitem("disable_mort_greff", &disable_mort_greff, 1, CB_NONE, "Wheter to disable growth efficency mortalitiy");
        // <-- PPX-Hydraulics


#ifdef HAVE_SMARTIO
		declareitem("suppress_daily_output", &so_options.supress_daily_output, 1, CB_NONE, "Wheter to suppress daily output or not");
		declareitem("suppress_monthly_output", &so_options.supress_monthly_output, 1, CB_NONE, "Wheter to suppress monthly output or not");
		declareitem("suppress_annually_output", &so_options.supress_annualy_output, 1, CB_NONE, "Wheter to suppress monthly output or not");
		declareitem("output_time_range", &strparam, 20, CB_SMARTOUT_TIMERANGE,
			"Output time types: (\"Scenario\", \"Spinup\", \"Full\", \"Custom\")");
		declareitem("year_begin", &so_options.year_begin, -1000, 5000, 1, CB_NONE, "year when the output should start");
		declareitem("year_end", &so_options.year_end, -1000, 5000, 1, CB_NONE, "year when the output should end");
		declareitem("output_data_format", &strparam, 20, CB_OUT_DATA_FORMAT,
			"Output data format (\"NC3_CDF1\", \"NC3_CDF2\", \"NC3_CDF5\", \"NC4_HDF5\")");
#endif
        ...
        // --> Hydraulics-PPX
        declareitem("alphaa_nlim", &alphaa_nlim,0.0, 1.0, 1, CB_NONE,
                        "Alpha_a parameter with nitrogen limitation turned on!");
        // <--> Hydraulics-PPX
		...
		declareitem("ifdetrendspinuptemp",&ifdetrendspinuptemp,1,CB_NONE,"If spinupdata should be detrended");
	...
	case BLOCK_PFT:
        ...
        // --> Hydraulics-PPX
        declareitem("isohydricity", &ppft->isohydricity, -0.5, 1.0, 1, CB_NONE,
                        "Fraction how isohydric (1) or anisohydric (0) a species should behave");
        declareitem("delta_psi_max", &ppft->delta_psi_max, 0.0, 10.0, 1, CB_NONE,
                        "Maximum forcing pressure that a plant should try to maintain");
        declareitem("cav_slope", &ppft->cav_slope, -1000.0, 0.0, 1, CB_NONE,
                        "Slope of the cavitation xylem response");
        declareitem("psi50_xylem", &ppft->psi50_xylem, -50.0, 0.0, 1, CB_NONE,
                        "Psi_50 of the species");
        declareitem("ks_max", &ppft->ks_max, 0, 1000.0, 1, CB_NONE,
                        "Specific maximum sapwood conductivity per stem crossectional area");
        declareitem("kr_max", &ppft->kr_max, 0, 1000.0, 1, CB_NONE,
                        "Specific maximum root conductivity");
        declareitem("kl_max", &ppft->kl_max, 0, 1000.0, 1, CB_NONE,
                        "Specific maximum leaf conductivity");
        declareitem("rtd", &ppft->rtd, 0, 1000.0, 1, CB_NONE,
                        "Root tissue density");
...
void plib_callback(int callback) {
	...
	case CB_WATERUPTAKE:
		...
		break;
            // --> PPX-Hydraulics
        case CB_HYDRAULICSYSTEM:
            if (strparam.upper() == "STANDARD") hydraulic_system = STANDARD;
            else if (strparam.upper() == "MONTEITH_SUP_DEM") hydraulic_system = MONTEITH_SUP_DEM;
            else if (strparam.upper() == "VPD_BASED_GC") hydraulic_system = VPD_BASED_GC;
            else {
                sendmessage("Error",
                            "Unknown water hydraulic system mode (valid types: \"STANDARD\", \"MONTEITH_SUP_DEM\", \"VPD_BASED_GC\")");
                plibabort();
            }
            break;
            // <-- PPX-Hydraulics
...
#ifdef HAVE_SMARTIO
		case CB_SMARTOUT_TIMERANGE:
			if (strparam.upper() == "SCENARIO") so_options.time_range = OutputTimeRange::Scenario;
			else if (strparam.upper() == "FULL") so_options.time_range = OutputTimeRange::Full;
			else if (strparam.upper() == "SPINUP") so_options.time_range = OutputTimeRange::Spinup;
			else if (strparam.upper() == "CUSTOM") so_options.time_range = OutputTimeRange::Custom;
			else {
				sendmessage("Error",
					"Unknown SmartOutput time range time (valid types: \"SCENARIO\", \"FULL\", \"SPINUP\",\"CUSTOM\")");
				plibabort();
			}
			break;

		case CB_OUT_DATA_FORMAT:
			if (strparam.upper() == "NC3_CDF1") so_options.data_format = nc_internal::DataFormat::NC3_CDF1;
			else if (strparam.upper() == "NC3_CDF2") so_options.data_format = nc_internal::DataFormat::NC3_CDF2;
			else if (strparam.upper() == "NC3_CDF5") so_options.data_format = nc_internal::DataFormat::NC3_CDF5;
			else if (strparam.upper() == "NC4_HDF5") so_options.data_format = nc_internal::DataFormat::NC4_HDF5;
			else {
				sendmessage("Error",
					"Unknown SmartOutput time range time (valid types: \"NC3_CDF1\", \"NC3_CDF2\", \"NC3_CDF5\",\"NC4_HDF5\")");
				plibabort();
			}
			break;
#endif
	...
	case CB_CHECKGLOBAL:
        ...
        // --> PPX-Hydraulics
        if (!itemparsed("hydraulic_system")) badins("hydraulic_system");
        // <-- PPX-Hydraulics
	...
	case CB_CHECKPFT:
		if (!checked_pft[ppft->name]) {
            ...
            if (hydraulic_system == hydraulicsystemtype::VPD_BASED_GC)
            {
                if (!itemparsed("isohydricity")) badins("isohydricity");
                if (!itemparsed("delta_psi_max")) badins("delta_psi_max");
                if (!itemparsed("cav_slope")) badins("cav_slope");
                if (!itemparsed("psi50_xylem")) badins("psi50_xylem");
                if (!itemparsed("ks_max")) badins("ks_max");
                if (!itemparsed("kr_max")) badins("kr_max");
                if (!itemparsed("kl_max")) badins("kl_max");
            }

...
double calc_relative_humidity(double temp, double specific_humidity, double pressure) {
	...
	if (pressure > 110000 || pressure < 10000) {
...
NetCDF_Common::NetCDF_Common() : cf_temp(0),
                                 cf_prec(0),
                                 cf_insol(0),
                                 // --> PPX-Hydraulics
                                 cf_vpd(0),
...
void NetCDF_Common::init_driver_files()
{
	...
	try {
		...
		if (param["file_vpd"].str != "") {
			cf_vpd = new GridcellOrderedVariable(param["file_vpd"].str, param["variable_vpd"].str);
		}
		...
		//RAI principle, need not delete it later.
		ncdf_ndep = std::make_unique<CfNdepInputConst>();
		ncdf_ndep->init();
	}
	...
	//Check if temperature is reduced
	if (cf_temp->is_reduced())
		is_reduced_ = true;

}

void NetCDF_Common::check_driver_files()
{
	...
	if (cf_vpd) {
		check_vpd_variable(cf_vpd);
	}

	if (cf_wind) {
		check_windspeed_variable(cf_wind);
	}
	...
	std::string prec_unit = cf_prec->get_units();

	if (prec_unit == "kg m-2 s-1")
		extensive_precipitation = false;
	else if (prec_unit == "kg m-2" || prec_unit == "mm/d")
		extensive_precipitation = true;
	else
	{
		fail("Wrong precipitation unit");
	}
}

void NetCDF_Common::init_landcover_and_management()
{
...
bool NetCDF_Common::load_data_from_files(double& lon, double& lat) {

	int landid = current_gridcell.landid;
	int rlon = current_gridcell.lon_index;
	int rlat = current_gridcell.lat_index;
	...
	if (cf_temp->is_reduced()) {
		if (!cf_temp->load_data_for(landid) ||
			...
			// --> PPX-Hydraulics
			(cf_vpd && !cf_vpd->load_data_for(landid)) ||
		...
		if (!cf_temp->load_data_for(rlon, rlat) ||
			...
			// --> PPX-Hydraulics
			(cf_vpd && !cf_vpd->load_data_for(rlon, rlat)) ||
...
void NetCDF_Common::load_spinup_data(const GuessNC::CF::GridcellOrderedVariable* cf_var, GenericSpinupData& spinup_data)
{
...
void NetCDF_Common::get_yearly_data(std::vector<double>& data, const GenericSpinupData& spinup, GuessNC::CF::GridcellOrderedVariable* cf_historic, int& historic_timestep)
{
...
void NetCDF_Common::get_yearly_data_no_spinup(std::vector<double>& data, double const_spinup_value,
	GuessNC::CF::GridcellOrderedVariable* cf_historic, int& historic_timestep)
{
	...
	if (is_daily(cf_historic)) {
		...
		while (current_day.year == date.year) {
			...
			if (earlier_day(current_day, calendar_year, cf_historic->get_date_time(0))) {
				...
				data[current_day.day] = const_spinup_value;
			}
			else {
				// Historical period
...
void NetCDF_Common::populate_daily_array(double* daily, const GenericSpinupData& spinup,
	GuessNC::CF::GridcellOrderedVariable* cf_historic, int& historic_timestep, double minimum, double maximum)
{
...
void NetCDF_Common::populate_daily_prec_array(long& seed)
{	// Get the data from spinup and/or historic
...
void NetCDF_Common::populate_daily_arrays(Gridcell& gridcell)
{
	...
	if (!is_daily(cf_temp) && weathergenerator == GWGEN) {
		...
		get_yearly_data(mwind, spinup_windspeed, cf_wind, historic_timestep_windspeed);
		double xmwind[12];
		for (int i = 0; i < 12; i++) {
			xmwind[i] = mwind[i];
		}
		...
		if (cf_relhum) {
			get_yearly_data(mrelhum, spinup_hurs, cf_relhum, historic_timestep_hurs);
			for (int i = 0; i < 12; i++) {
				xmrhum[i] = mrelhum[i];
			}
		}
		...
		if (cf_wind) {
			populate_daily_array(dwindspeed, spinup_windspeed, cf_wind, historic_timestep_windspeed, 0);
		}

        if (cf_vpd) {
            populate_daily_array(dvpd, spinup_vpd, cf_vpd, historic_timestep_vpd, 0);
        }

		if (cf_relhum) {
			populate_daily_array(dhurs, spinup_hurs, cf_relhum, historic_timestep_hurs, 0);
		}
	}
	...
	for (int i = 0; i < date.year_length(); ++i) {
		...
		if ((cf_pres && cf_specifichum) && !cf_relhum) {
			...
			dhurs[i] = calc_relative_humidity(dtemp[i], dspecifichum[i], dpres[i]);
		}
	...
	if (cf_wind) {
		spinup_windspeed.nextyear();
	}

	if (cf_relhum) {
		spinup_hurs.nextyear();
	}
		
	double daily_dNH4dep = ncdf_ndep->get_ndep_nhx(gridcell.get_lon(), gridcell.get_lat()) / 1000.0 / 365.0;
	double daily_dNO3dep = ncdf_ndep->get_ndep_noy(gridcell.get_lon(), gridcell.get_lat()) / 1000.0 / 365.0;
	for (int i = 0; i < date.year_length(); ++i) {
		dNH4dep[i] = daily_dNH4dep;
		dNO3dep[i] = daily_dNO3dep;
	}
}
...
std::vector<GuessNC::CF::GridcellOrderedVariable*> NetCDF_Common::all_variables() const
{
...
double NetCDF_Common::get_vpd_by_temp_and_hurs(double temperature, double hurs)
{
	double T = temperature;
	return -0.61078 * std::exp(17.27 * T / (T + 237.3)) * (1.0 - hurs / rel_hum_multiplier);
	
}

int NetCDF_Common::parseLine(char* line)
{
...
int NetCDF_Common::get_virtual_memory()
{
...
int NetCDF_Common::get_physical_memory()
{
	//Note: this value is in KB!
	FILE* file = fopen("/proc/self/status", "r");
	int result = -1;
...
void NetCDF_Common::check_temp_variable(const GuessNC::CF::GridcellOrderedVariable* cf_var)
{
	...
	if (cf_var->get_units() == "K") {
		reduce_celsius_to_kelvin = false;
	}

	else if (cf_var->get_units() == "celsius") {
		reduce_celsius_to_kelvin = true;
	}

	else {
		//fail("Temperature variable doesn't seem to be in Kelvin or Celsius");
	}
}

double NetCDF_Common::max_insolation(insoltype instype)
{
	if (instype == SUNSHINE) {
		return 100;
	}
...
insoltype NetCDF_Common::cf_standard_name_to_insoltype(const std::string& standard_name)
{
	if (standard_name == "surface_downwelling_shortwave_flux_in_air" ||
		...
		return SWRAD_TS; // To avoid compiler warning
	}
}

// Checks if a DateTime is at the first day of the year
bool NetCDF_Common::first_day_of_year(GuessNC::CF::DateTime dt) {
	return dt.get_month() == 1 && dt.get_day() == 1;
}

// Checks if a DateTime is in January
bool NetCDF_Common::first_month_of_year(GuessNC::CF::DateTime dt) {
	return dt.get_month() == 1;
}
// Compares a Date with a GuessNC::CF::DateTime to see if the Date is on an earlier day
bool NetCDF_Common::earlier_day(const Date& date, int calendar_year,
...
// have been called)
bool NetCDF_Common::later_day(const Date& date,
	const GuessNC::CF::DateTime& date_time) {
	std::vector<int> d1(3), d2(3);
...
bool NetCDF_Common::is_daily(const GuessNC::CF::GridcellOrderedVariable* cf_var) {

	// Check if first and second timestep is one day apart

	GuessNC::CF::DateTime dt1 = cf_var->get_date_time(0);
	GuessNC::CF::DateTime dt2 = cf_var->get_date_time(1);
...
GuessNC::CF::DateTime NetCDF_Common::last_day_to_simulate(const GuessNC::CF::GridcellOrderedVariable* cf_var) {
...
void NetCDF_Common::check_vpd_variable(const GuessNC::CF::GridcellOrderedVariable* cf_var) {
	//if (cf_var && (cf_var->get_standard_name() != "Vapor pressure deficit" && cf_var->get_standard_name() != "vpd") ) {
	if (cf_var && (cf_var->get_standard_name() != "Vapor" && cf_var->get_standard_name() != "vpd") ) {
		fail("VPD variable doesn't seem to contain vpd data");
	}
	if (cf_var->get_units() != "kPa") {
		fail("VPD variable doesn't seem to be in kPa");
	}
}

void NetCDF_Common::check_relhum_variable(const GuessNC::CF::GridcellOrderedVariable* cf_var)
{
    ...
    std::string unit = to_lower(cf_var->get_units());
    if (unit == "" || unit == "1"){
        rel_hum_multiplier = 1.0;
    }
    else if (unit == "%" || unit == "Percent"){
        rel_hum_multiplier = 100.0;
    }
    else{
        fail("Relative Humidity must be dimensionless or in % ");
    }
	
}

void NetCDF_Common::check_windspeed_variable(const GuessNC::CF::GridcellOrderedVariable* cf_var) {
...
void NetCDF_Common::check_prec_variable(const GuessNC::CF::GridcellOrderedVariable* cf_var) {
...
void NetCDF_Common::check_insol_variable(const GuessNC::CF::GridcellOrderedVariable* cf_var) {
	if (cf_var->get_standard_name() != "surface_downwelling_shortwave_flux_in_air" &&
		cf_var->get_standard_name() != "ta" &&
...
void NetCDF_Common::check_wetdays_variable(const GuessNC::CF::GridcellOrderedVariable* cf_var) {
...
void NetCDF_Common::check_compatible_timeseries(const GuessNC::CF::GridcellOrderedVariable* var1,
...
void NetCDF_Common::check_compatible_timeseries(const std::vector<GuessNC::CF::GridcellOrderedVariable*> variables) {
...
void NetCDF_Common::check_same_spatial_domains(const std::vector<GuessNC::CF::GridcellOrderedVariable*> variables) {

...
NcInputParallelAuto::NcInputParallelAuto(InputModuleType type):InputModule(type)
{

}


NcInputParallelAuto::~NcInputParallelAuto()
{
	global_log.flush();
	global_log.close();
}

void NcInputParallelAuto::init(const CommandLineArguments& args) {
	...
	// Get gridlist
	mpi_gridlist_handler = std::make_unique<nc_guess_internal::MPI_Gridlist_Auto>(is_reduced_, *cf_temp, global_log, verbose);
...
bool NcInputParallelAuto::getgridcell(Gridcell& gridcell)
{
	...
	{		
		...
		while (!is_finished_ && !load_data_from_files(current_gridcell.lon,
			current_gridcell.lat))
		{
			mpi_gridlist_handler->SendDoneToRoot();
			mpi_gridlist_handler->ReceiveNextCoord();
...
bool NcInputParallelAuto::getclimate(Gridcell& gridcell)
{
	if (is_first_year)
	{
		is_first_year = false;
...
void NcInputParallelAuto::getlandcover(Gridcell& gridcell)
{
	landcover_input.getlandcover(gridcell);
	landcover_input.get_land_transitions(gridcell);
}

double& NcInputParallelAuto::GetInputMs()
{
	return elapsed_input_ms;
}

void NcInputParallelAuto::SendReceiveHandling()
{
	mpi_gridlist_handler->SendReceiveHandling();
}
#endif
#endif
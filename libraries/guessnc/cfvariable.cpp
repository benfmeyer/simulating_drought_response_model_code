...
#ifdef HAVE_SMARTIO
std::vector<CoordI> GridcellOrderedVariable::get_all_coords() const
{
	size_t ncoords = get_n_stations();

	std::vector<double> lons(ncoords);
	std::vector<double> lats(ncoords);
	size_t offset[1] = { 0 };
	size_t count[1] = { ncoords };
	int status = 0;
	status = nc_get_vara_double(ncid_file, ncid_lon, offset, count, &lons.front());
	status = nc_get_vara_double(ncid_file, ncid_lat, offset, count, &lats.front());
	handle_error(status, "Error loading netcdffile");

	std::vector<CoordI> coords(ncoords);
...
size_t GridcellOrderedVariable::get_n_stations() const
{
	size_t length;
	int status = nc_inq_dimlen(ncid_file, landid_dimension_index, &length);
	handle_error(status, "No station dimension found");
	return length;
}

std::vector<double> GridcellOrderedVariable::get_all_lons() const
{
	size_t nlons;
	int status = nc_inq_dimlen(ncid_file, ncid_lon, &nlons);
	handle_error(status, "No lon dimension found");

	std::vector<double> lons(nlons);
	size_t offset[1] = { 0 };
	size_t count[1] = { nlons };
	status = nc_get_vara_double(ncid_file, ncid_lon, offset, count, &lons.front());

	handle_error(status, "Could not load lon data");
	return lons;
}

std::vector<double> GridcellOrderedVariable::get_all_lats() const
{
	size_t nlats;
	int status = nc_inq_dimlen(ncid_file, ncid_lat, &nlats);
	handle_error(status, "No lat dimension found");

	std::vector<double> lats(nlats);
	size_t offset[1] = { 0 };
	size_t count[1] = { nlats };
	status = nc_get_vara_double(ncid_file, ncid_lat, offset, count, &lats.front());

	handle_error(status, "Could not load lat data");
	return lats;
}

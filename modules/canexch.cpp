...
void water_supply_std(Individual& indiv, const Day& day)
{
    Patchpft& ppft = indiv.patchpft();
...
//Hydro dynamics water supply function, identical for all hydraulic systems except standard hydraulics
void water_supply_hydraulics(Individual& indiv)
{
    // Update the hydraulic system
    indiv.hydraulic_module->Update();

    // Set indiv aet to 0.0 to be sure we add up 0.0 if the individual is not alive
    indiv.aet = 0.0;

    // also set annual aet to zero
    if (date.day == 0)
        indiv.aaet = 0.0;

    // Update aet with the total plant transpiration
    if (indiv.alive)
        indiv.aet = indiv.hydraulic_module->get_et_plant_total();
...
void aet_water_stress(Patch& patch, Vegetation& vegetation, const Day& day) {
    ...
    while (vegetation.isobj) {
        ...
        if (hydraulic_system == hydraulicsystemtype::STANDARD || indiv.pft.lifeform == GRASS)
            water_supply_std(indiv, day);
        else
            water_supply_hydraulics(indiv);
...
void npp(Patch& patch, Climate& climate, Vegetation& vegetation, const Day& day) {
	...
	while (vegetation.isobj) {
        ...
        // --> PPX-Hydraulics
        // Intialize with maximum in case we do not have any water stress
		...
		if (indiv.wstress) {
            ...
            {
                // Water stress based on the hydraulics estimate for canopy conductance
                assimilation_wstress(pft, pftco2, temp, par, hours, indiv.fpar, indiv.fpc,
                                     indiv.hydraulic_module->get_gC(), phot.vm, phot, lambda,
                                     indiv.nactive / indiv.nextin, ifnlim, moss_wtp_limit, graminoid_wtp_limit, inund_stress);
            }


            // Update lambda for each Individual
            indiv.lambda = lambda;
            // <-- PPX-Hydraulics
        }

...
// --> PPX-Hydraulics
// Calculates the soil water potential according to Saxton et al. 1986
void psi_soil(Patch& patch, Soil& soil)
{

    for (int s = 0; s < NSOILLAYER; ++s) {

        double wcont = soil.get_layer_soil_water(s);
        double awc = soil.soiltype.awc[s];
        double wcont_tot = soil.soiltype.wilting_point + wcont * awc*patch.fpc_rescale/ soil.Dz[s + 7];
        //double wcont_tot = soil.soiltype.wp[s] + wcont * awc*patch.fpc_rescale;
        // Saxton et al. 1986 eq.2
        soil.psi_s[s] = soil.soiltype.shapeA * pow(wcont_tot, soil.soiltype.shapeB);
    }
...
void soilwater(Patch& patch, Climate& climate) {
    ...
    // --> PPX-Hydraulics
    // Calculates the soil water potential

...
/// --> PPX Hydraulics
// Calculates shape parameter essential for soil water pot calculations
// Todo Move this down to the other similar parameters. there are shape parameters already
void water_pot_parameters(Soiltype& soiltype)
{

    // Saxton et al. 1986 (eq. 5 & 6)
    // 10 to transform into MPa
		double clay_percent = soiltype.clay_frac * 100.0;
		double sand_percent = soiltype.sand_frac * 100.0;
		soiltype.shapeA = -exp(-4.396 - 0.0715*clay_percent - 4.88*0.0001
			*pow(sand_percent, 2) - 4.285*0.00001*pow(sand_percent, 2)*clay_percent) / 10.0;
        soiltype.shapeB = -3.14 - 0.00222*pow(clay_percent, 2) - 3.484*0.00001
		  *pow(sand_percent, 2)*clay_percent;

    // Calculate actual total volumetric water content (wcont_tot) in each soil layer,
    // used below to calculate soil water potential following Saxton et al. 1984
    // In standard LPJ, water content is fraction of available water holding capacity (awc).
    // Thus, total volumetric water content is: wcont (standard LPJ)
    // plus volumetric water content at wilting point, which in turn is
    // the saturated volumetric water content (SVWC, Cosby et al. 1984) - awc

    // Volumetric water content at permantant wilting point (inversion of Saxton et al. 1986 eq. 2)
    // Psi_PWP=10e-4.2 hPa following Schaeffer et al. 2000 (pinus sylvestris)
    soiltype.PWPVWC = pow(WILTING_POINT / soiltype.shapeA, 1.0 / soiltype.shapeB); // 1.5 used before for VEMAP
...
void SoilInput::get_soil_mineral(double lon, double lat, Gridcell& gridcell) {
    ...
    soiltype.wilting_point = soilprop.wilting_point;

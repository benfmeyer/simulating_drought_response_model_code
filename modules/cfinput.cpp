...
CFInput::CFInput(InputModuleType type) : InputModule(type),
	 cf_temp(0),
	  cf_prec(0),
	  cf_insol(0),
	  cf_wetdays(0),
...
void CFInput::init(const CommandLineArguments& args) {
...
void CFInput::populate_daily_arrays(Gridcell& gridcell) {
	...
	for (int i = 0; i < date.year_length(); ++i) {
		...
		if ( (cf_pres && cf_specifichum) && !cf_relhum ) {
			// compute relative humidity for BLAZE and VPD_BASED_GC (hydraulics)
		...
		} else if (hydraulic_system == VPD_BASED_GC && !cf_relhum) {
...
bool CFInput::getclimate(Gridcell& gridcell) {
    ...
    if (cf_vpd)
        ...
        climate.vpd = get_vpd_by_temp_and_hurs(dtemp[date.day],drelhum[date.day]);
    ...
    if (cf_wind){
        climate.windspeed = dwind[date.day];
    }
...
double CFInput::get_vpd_by_temp_and_hurs(double temperature, double rel_hum) {
    double T = temperature;
    return -0.61078 * std::exp(17.27 * T / (T + 237.3)) * (1.0 - rel_hum);
}

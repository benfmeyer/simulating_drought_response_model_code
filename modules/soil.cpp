...
void Soil::hydrology_lpjf_twolayer(const Climate& climate, double fevap) {
	...
	patch.et_total = patch.et_plant + patch.et_soil;
...
// return the available water content for each layer / varies with soil type
double Soil::get_soil_water_frac(int layer) const{

	if (layer < 0 || layer > NSOILLAYER-1 || layer != int(layer)){
		return 0.0;}
	else{
		return  soiltype.awc[layer];
	}
}

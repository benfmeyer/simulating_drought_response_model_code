...
public:
	CFInput(InputModuleType type);

	~CFInput();

	void init(const CommandLineArguments& args);
	...
	static const int NYEAR_SPINUP_DATA=10;
...
private:
    ...
    /// Daily vapor pressure deficit range for current gridcell and current year (kPA)
    ...
    /// <param name="hurs"> Relative humidity (dimensionless) </param>
    /// <returns>VPD in kPA (negative units) ß</returns>
    double get_vpd_by_temp_and_hurs(double temperature, double rel_hum);
};

...
// --> PPX-Hydraulics
/**
* \brief cavitation reduction function. IMPORTANT NOTICE: This function has to be called AFTER the cmass_sap has been incremented!
* Otherwise cmass_sap would not account for the new total sapwood and we would not have a valid convex combination
* \param indiv
* \param cmass_sap_inc
*/
void cav_xylem_reduction(Individual& indiv, const double cmass_sap_inc)
{
    // Cavitation reduction happens because of two reasons:
    // 1. The total amount of cavitation affecting sapwood decreases
    // when sapwood turns into heartwood.
    // This is however done passively just by reducing the amount of total sapwood within allocation
    // Todo: Introduce different strategies at this point to increase this sapwood turnover

    // 2. The generation of new sapwood from assimilation will have no cavitated xylems and
    // this reduce the total fraction of cavitation.
    // this is only be done if we have a possitive sapwood increment
    if (cmass_sap_inc > 0.0)
    {
        // Retrieve cavitated xylem references
        double cav_xylem = indiv.hydraulic_data.kappa_s_min;
        double cav_xylem_prev = indiv.hydraulic_data.kappa_s_today;

        // Calculate the propotion of healty sapwood
        const double frac_cmass_new = cmass_sap_inc * indiv.densindiv / indiv.cmass_sap;

        // The new new xylem cavitation is then estimated via a convex combination of cavitated xylem and healthy xylem
        // However, the second term of the convex combination is zero, because healthy means 0.0, and multipling by 0.0 = 0.0
        cav_xylem = cav_xylem * frac_cmass_new; // + (1.0 - frac_cmass_new) * cmass_new;

        // The daily running value of the xylem cavitation is set to the actual xylem cavitation
        cav_xylem_prev = cav_xylem;

        indiv.hydraulic_data.kappa_s_min = cav_xylem;
        indiv.hydraulic_data.kappa_s_today = cav_xylem_prev;
    }
		...
		if (negligible(indiv.densindiv))
			...
			if (!killed) {
				...
				if (indiv.pft.lifeform == TREE) {
                    ...
                    // --> PPX-Hydraulics
                    // Reduce the xylem cavitation via new sapwood
                    // This function has to be called after cmass_sap has already be increased
                    if (hydraulic_system != hydraulicsystemtype::STANDARD && indiv.pft.lifeform != GRASS)
                        cav_xylem_reduction(indiv, cmass_sap_inc);
                    // <-- PPX-Hydraulics
